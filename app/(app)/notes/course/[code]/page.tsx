...
async function loadCourseContext(rawCode: string) {
    ...
    const effectiveCode = courseDetail?.code ?? tagCourse?.code ?? normalized;
    const effectiveTitle = courseDetail?.title ?? tagCourse?.title ?? normalized;
    ...
    return {
        ...
        noteCountHint: courseDetail?.noteCount ?? 0,
    };
}
...
export async function generateMetadata({
    ...
    const noteCount = await getCourseNotesCount({
            ...
            <div className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-3 py-6 sm:px-6 lg:px-10 lg:py-10">
                <header className="flex flex-col gap-4">
                    <div className="flex flex-wrap items-center gap-1.5 text-sm text-black/50 dark:text-[#D5D5D5]/50">
                        <Link href="/notes" className="hover:text-black dark:hover:text-[#D5D5D5]">
                            Notes
                        </Link>
